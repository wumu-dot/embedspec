...
| 检查项 | 文件位置 | 当前值（STM32示例） | 新芯片应填 |
|--------|----------|-------------------|-----------|
| 调试器配置 | `firmware/openocd.cfg` | `source [find target/stm32f4x.cfg]` | 改为目标芯片配置，如 `nrf52.cfg`、`rp2040.cfg` |
| 烧录命令 | `.skills/flash.sh` | `openocd -f openocd.cfg -c "program ..."` | 确认烧录方式兼容，可能需要 pyocd、JLink 等 |
| RTOS | `DOC-STATE` | `FreeRTOS/CMSIS-RTOS_v2` | 改为 `Zephyr` / `RT-Thread` / `None` |
| 构建系统 | `DOC-STATE` | `Makefile+arm-none-eabi-gcc` | 改为实际工具链 |
| 时钟源 | `DOC-STATE` | `HSE=8000000` | 填主时钟频率，无HSE填 `N/A` 或 `INTOSC` |

> 操作流程：按上表逐项改完 → 运行 `scripts/check-doc-drift.sh` 校验 DOC-STATE → 通过则适配完成。
