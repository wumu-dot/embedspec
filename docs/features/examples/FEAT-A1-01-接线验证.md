...
- [x] AC 逐条打勾，全部通过
- [x] 回填：回填「维护与调试」章节 + 父FEAT 项目表 FEAT-A1-01 行置 🟢 + INDEX.md 子行置 🟢 + 维护地图追加一行
...
| 文件 | 改动类型 | 说明 |
|------|---------|------|
| firmware/App/test/wiring_check.c | 新增 | 接线自检测试代码 |

### 4.2 调试要点
- 逻辑分析仪抓取 I2S/PDM 时序，确认信号线无虚焊
- 接线异常时用 /debug 抓 backtrace 定位

### 4.3 测试入口
- `./scripts/ci_local.sh`（对应 AC-03）
- 接线自检用例（对应 AC-01 / AC-09）
